...
type Log struct {
	LogID  string      `boil:"log_id" json:"log_id" toml:"log_id" yaml:"log_id"`
...
var (
	logColumns               = []string{"log_id", "page_id", "type", "time"}
	logColumnsWithoutDefault = []string{"page_id", "type", "time"}
...
func (o *Log) PageG(mods ...qm.QueryMod) pageQuery {
	return o.Page(boil.GetDB(), mods...)
}

// Page pointed to by the foreign key.
func (o *Log) Page(exec boil.Executor, mods ...qm.QueryMod) pageQuery {
...
func (logL) LoadPage(e boil.Executor, singular bool, maybeLog interface{}) error {
	var slice []*Log
	var object *Log
...
// SetPageG of the log to the related item.
// Sets o.R.Page to related.
// Adds o to related.R.Logs.
// Uses the global database handle.
func (o *Log) SetPageG(insert bool, related *Page) error {
	return o.SetPage(boil.GetDB(), insert, related)
}

// SetPageP of the log to the related item.
// Sets o.R.Page to related.
// Adds o to related.R.Logs.
// Panics on error.
func (o *Log) SetPageP(exec boil.Executor, insert bool, related *Page) {
	if err := o.SetPage(exec, insert, related); err != nil {
		panic(boil.WrapErr(err))
	}
}

// SetPageGP of the log to the related item.
// Sets o.R.Page to related.
// Adds o to related.R.Logs.
// Uses the global database handle and panics on error.
func (o *Log) SetPageGP(insert bool, related *Page) {
	if err := o.SetPage(boil.GetDB(), insert, related); err != nil {
		panic(boil.WrapErr(err))
	}
}

// SetPage of the log to the related item.
// Sets o.R.Page to related.
// Adds o to related.R.Logs.
func (o *Log) SetPage(exec boil.Executor, insert bool, related *Page) error {
	var err error
	if insert {
	...
	values := []interface{}{related.PageID, o.LogID}
...
// Uses the global database handle.
func (o *Log) RemovePageG(related *Page) error {
...
// Removes o from all passed in related items' relationships struct (Optional).
// Panics on error.
func (o *Log) RemovePageP(exec boil.Executor, related *Page) {
...
// Removes o from all passed in related items' relationships struct (Optional).
// Uses the global database handle and panics on error.
func (o *Log) RemovePageGP(related *Page) {
...
// Sets o.R.Page to nil.
// Removes o from all passed in related items' relationships struct (Optional).
func (o *Log) RemovePage(exec boil.Executor, related *Page) error {
	var err error
	...
	for i, ri := range related.R.Logs {
		if o.PageID.String != ri.PageID.String {
			continue
		}

		ln := len(related.R.Logs)
		if ln > 1 && i < ln-1 {
			related.R.Logs[i] = related.R.Logs[ln-1]
		}
		related.R.Logs = related.R.Logs[:ln-1]
		break
	}
	return nil
}
...
func FindLogG(logID string, selectCols ...string) (*Log, error) {
	return FindLog(boil.GetDB(), logID, selectCols...)
}

// FindLogGP retrieves a single record by ID, and panics on error.
func FindLogGP(logID string, selectCols ...string) *Log {
...
func FindLog(exec boil.Executor, logID string, selectCols ...string) (*Log, error) {
	logObj := &Log{}
...
func FindLogP(exec boil.Executor, logID string, selectCols ...string) *Log {
...
func (o *Log) Insert(exec boil.Executor, whitelist ...string) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"logs\" (\"%s\") VALUES (%s)", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.IndexPlaceholders, len(wl), 1, 1))
		} else {
...
func (o *Log) Update(exec boil.Executor, whitelist ...string) error {
	...
	if !cached {
		wl := strmangle.UpdateColumnSet(logColumns, logPrimaryKeyColumns, whitelist)
		if len(whitelist) == 0 {
...
func (o LogSlice) UpdateAll(exec boil.Executor, cols M) error {
	...
	sql := fmt.Sprintf(
		"UPDATE \"logs\" SET %s WHERE (\"log_id\") IN (%s)",
		strmangle.SetParamNames("\"", "\"", 1, colNames),
		strmangle.Placeholders(dialect.IndexPlaceholders, len(o)*len(logPrimaryKeyColumns), len(colNames)+1, len(logPrimaryKeyColumns)),
	)
...
func (o LogSlice) DeleteAll(exec boil.Executor) error {
	...
	sql := fmt.Sprintf(
		"DELETE FROM \"logs\" WHERE (%s) IN (%s)",
		strings.Join(strmangle.IdentQuoteSlice(dialect.LQ, dialect.RQ, logPrimaryKeyColumns), ","),
		strmangle.Placeholders(dialect.IndexPlaceholders, len(o)*len(logPrimaryKeyColumns), 1, len(logPrimaryKeyColumns)),
	)
...
func (o *LogSlice) ReloadAll(exec boil.Executor) error {
	...
	sql := fmt.Sprintf(
		"SELECT \"logs\".* FROM \"logs\" WHERE (%s) IN (%s)",
		strings.Join(strmangle.IdentQuoteSlice(dialect.LQ, dialect.RQ, logPrimaryKeyColumns), ","),
		strmangle.Placeholders(dialect.IndexPlaceholders, len(*o)*len(logPrimaryKeyColumns), 1, len(logPrimaryKeyColumns)),
	)
...
func LogExists(exec boil.Executor, logID string) (bool, error) {
	var exists bool
...
func LogExistsP(exec boil.Executor, logID string) bool {
